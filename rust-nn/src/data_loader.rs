...
impl MnistDataset {
    ...
    pub fn get_batch(&self, batch_indices: &[usize]) -> (TensorHandle, Vec<u8>) {
        ...
        let image_tensor = Tensor::with_shape_f32(
            image_data,
            vec![batch_size, self.image_size],
            false,
        );
        
        // Get corresponding labels
        let labels: Vec<u8> = batch_indices.iter()
...
pub fn check_mnist_dataset() -> io::Result<PathBuf> {
    ...
    let all_exist = files.iter()
        .all(|file| data_dir.join(file).exists());
...
pub fn load_mnist_images(file_path: &Path) -> io::Result<(Vec<Vec<f32>>, usize)> {
...
pub fn load_mnist_dataset(data_dir: &Path) -> io::Result<(MnistDataset, MnistDataset)> {
    ...
    let (train_images, image_size) = match load_mnist_images(&train_images_path) {
        Ok(result) => {
    ...
    let (test_images, _) = match load_mnist_images(&test_images_path) {
        Ok(result) => {
